...
def Not_True_Strike(msg, pOneInfo, pTwoInfo, pOnepMod, pTwopMod, pOnecMod, pTwocMod, pOnedMod, pTwodMod, pOnemMod,
    ...
    if total < totalAC or hit == 1:
        msg.append(pOneInfo['name'] + " rolled a [color=red]" + str(total) + "[/color] to hit an AC [color=green]" +
                   ...
                   pTwoInfo['name'] + "'s turn. Type: [color=pink]!usefeat <feat>[/color] if you wish to use a feat.")
        ...
        if totalBlur != 0 and blur <= totalBlur:
            ...
            if pOneFeatUsed[0] == "reckless abandon":
                ...
                msg.append(pOneInfo['name'] + " did [color=red]" + str(damageToPOne) + "[/color] damage to "
                           "themselves, to deal an additional [color=red]" + str(damageToPTwo) +
            ...
            if pTwoFeatUsed[0] == "quick strike":
                ...
                msg.append(pTwoInfo['name'] + " used [color=yellow]'quick strike,'[/color] managing to do an "
                           "additional [color=red]" + str(pTwoQuickDamage) + "[/color] hp of damage.")
                ...
                pTwoQuickDamage = quickDamage
                pOneCurrentHP = pOneCurrentHP - pTwoQuickDamage
                msg.append(pTwoInfo['name'] + " used [color=yellow]'riposte,'[/color] managing to do an "
                           "additional [color=red]" + str(pTwoQuickDamage) + "[/color] hp of damage.")
                pTwoRiposte = 1
            if total < 1:
                total = 1
            ...
            if pTwoTempDR != 0:
                msg.append(pTwoInfo['name'] + "'s [color=yellow]lifeleech[/color] effect absorbed [color=red]" +
                           str(pTwoTempDR) + "[/color] damage.")
            ...
            msg.append(pOneInfo['name'] + " did [color=red]" + str(totalDamage) + "[/color] points of damage." +
            ...
            if pOneFeatUsed[0] == "lifeleech" and damage > 0:
                ...
                msg.append(pOneInfo['name'] + " has gained [color=red]" + str(pOneTempDR) + " DR[/color]"
                ...
                pOneTempDR = random.randint(pOneFeatUsed[1][0], pOneFeatUsed[1][1])
                msg.append(pOneInfo['name'] + "has gained [red]" + int(pOneTempDR) + " DR[/color]"
                           " until their next turn.")

            # check to see if player has evasion.
            if "evasion" in pTwoInfo['feats taken'] and pTwoEvade == 1:
                bEvasion = True
                msg.append(pTwoInfo['name'] + " has an evasion available for use. Type [color=pink]!evasion"
                           "[/color] to use, or type [color=pink]!pass[/color]")
