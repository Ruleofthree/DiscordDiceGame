...
def True_Strike(msg, pOneInfo, pTwoInfo, pOnepMod, pTwopMod, pOnecMod, pTwocMod, pOnedMod, pTwodMod, pOnemMod, pTwomMod,
    ...
    critical, count, featToken, bGameTimer, token, totalDamage, new_token ):

    msg.append(pOneInfo['name'] + " used the feat 'True Strike.' And forgoes the need to determine if hit was success.")
    ...
    # if attacker use 'staggering blow' half damage done.
    if pTwoFeatUsed[0] == "staggering blow":
        msg.append(pTwoInfo['name'] + " used the feat 'staggering blow', halving " + pOneInfo['name'] + "'s damage roll")
    ...
    if damage < 1:
        damage = 1
    total = int(damage + pOneModifier + pMod - cMod + bonusHurt)

    # if attacker has 'reckless abandon' apply bonus damage
    ...
    # if defender used 'quick strike', 'improved quick strike', or 'greater quick strike', apply the return
    # damage here
    ...
    if pTwoFeatUsed[0] == "quick strike":
        ...
        msg.append(pTwoInfo['name'] + " used 'quick strike,' managing to do an additional " + str(pTwoQuickDamage) +
                   " hp of damage.")
        ...
        # roll damage for defender, and multiply it by desired amount
        damage = random.randint(int(pTwoMinimum), int(pTwoMaximum))
        modifier = (pTwoModifier + pMod - cMod)
        quickDamage = int(damage * float(pTwoFeatUsed[1][0]) + modifier)
        # Ensure damage is always at least 1hp and print out result
        if quickDamage < 1:
            quickDamage = 1
        pTwoQuickDamage = quickDamage
        pOneCurrentHP = pOneCurrentHP - pTwoQuickDamage
        msg.append(pTwoInfo['name'] + " used 'riposte,' managing to do an additional " + str(pTwoQuickDamage) +
                   " hp of damage.")
    ...
    # check to see if defender has evasion.
    ...
    # check to see if defender has deflect.
    ...
    if bEvasion and bDeflect is False:
        # Determine if Quick Strike was used by defender and apply damage
