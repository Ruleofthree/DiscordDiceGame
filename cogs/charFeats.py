...
class Feats(commands.Cog):
    ...
    @commands.command()
    @commands.dm_only()
    async def feathelp(self, ctx, *, arg):
        featDictionary = featDict()[0]
        featList = featDict()[1]
        private = ctx.author.send
        answer = arg
    ...
    async def name_feathelp(self, ctx, error):
        ...
        #         super().PRI(character, "You have no feat slots left to select a new feat")
        ...
        #         reqFeats = featDictionary[0][answer]['requirements'][4]
        #         if reqLevel > level:
        #             super().PRI(character, "You are not the required level for this feat.")
        #         elif reqStr > strength:
        #             super().PRI(character, "You do not have the required strength for this feat.")
        #         elif reqDex > dexterity:
        #             super().PRI(character, "You do not have the required dexterity for this feat.")
        #         elif reqCon > constitution:
        #             super().PRI(character, "You do not have the required constitution for this feat.")
        #         elif reqFeats not in hasTaken and reqFeats != "none":
        #             super().PRI(character, "You do hot have the required prerequisites to take this feat.")
        #         elif answer not in hasTaken:
        #             super().PRI(character, answer + " has been added to your character sheet.")
        #             remainingFeats -= 1
        ...
        #                 hasTaken.pop(index)
        #
        #             with open(charFolder + character + ".txt", "r+", encoding="utf-8") as file:
